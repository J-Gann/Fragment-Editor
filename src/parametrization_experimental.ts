...
export class PyPaExp {
    ...
    static executeScript(script: string): Promise<string | any> {
        var promise = new Promise((resolve, reject) => {
            ...
            fs.writeFile(filePath, script, (err: any) => {
                if (!err) {
                    exec('python ' + filePath, (error: any, stdout: string, stderr: string) => {
                        if (error) {
                            reject(error);
                        } else if (stderr) {
                            reject(stderr);
    ...
    static parametrize(textDocument: vscode.TextDocument, selection: vscode.Selection): Promise<{ body: string, placeholders: string }> {
        ...
        var promise = new Promise<{ body: string, placeholders: string } | any>((resolve, reject) => {
            PyPaExp.getAstexport()
                .then(() => PyPaExp.getPlaceholders(textDocument.getText(), selection, preDefinedNames))
                .then((documentPlaceholders: Placeholder[]) => {
                    placeholdersDocument = documentPlaceholders;
                    var snippetPlaceholders = PyPaExp.placeholderIndicesToSnippet(documentPlaceholders, selection);
                    return snippetPlaceholders;
                })
                .then((snippetPlaceholders: Placeholder[]) => PyPaExp.sortPlaceholders(snippetPlaceholders))
                .then((sortedSnippetPlaceholders: Placeholder[]) => {
                    placeholdersSnippet = sortedSnippetPlaceholders;
                    return PyPaExp.parametrizedSnippet(textDocument, selection, sortedSnippetPlaceholders);
                })
                .then((parametrizedSnippet: string) => {
                    body = parametrizedSnippet;
                    return PyPaExp.createScript(textDocument, selection, placeholdersDocument);
                })
                .then((script: string) => PyPaExp.executeScript(script))
                .then((output: string) => PyPaExp.parseScriptOutput(output))
                .then((typeDefinitions: string[]) =>  PyPaExp.assignDatatypes(typeDefinitions, placeholdersSnippet))
                .then((placeholders: Placeholder[]) => {
                    placeholdersSnippet = placeholders;
                    resolve(PyPaExp.formatResult(placeholdersSnippet, body));
                })
                .catch((err) => {
                    reject(err);
                })
