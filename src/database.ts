    ...
    loadFragmentsFromExternal(): void {
        ...
        const news = res.values.filter((element: any[]) => !element[9] || element[9] === "parametrize").forEach((element: any[]) => {
            ...
            if (element[9] === "parametrize" && scope === 'python') {
                ...
                */
                const filePath: string = path.join(FragmentProvider.context.extensionPath, 'tmp', Math.random().toString());
                require('fs-extra').writeFile(filePath, body)
                    .then(() => {
                        vscode.workspace.openTextDocument(filePath)
                            .then((textDocument: vscode.TextDocument) => {
                                const lastLine = textDocument.lineCount - 1;
                                const lastChar = textDocument.lineAt(lastLine).text.length - 1;
                                var selectionEnd = new vscode.Position(lastLine, lastChar);
                                const selection: vscode.Selection = new vscode.Selection(new vscode.Position(0, 0), selectionEnd);
                                PyPa.parametrizeWithDatatypes(textDocument, selection)
